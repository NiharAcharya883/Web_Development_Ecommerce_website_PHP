...
<body>
    ...
    $gender = $_GET['gender'];
    $Jeans_color = "black";
    $sql = "Select * from `Jeans` where `gender` = '$gender'";
    $result = mysqli_query($conn,$sql);
    }
    elseif($_SERVER["REQUEST_METHOD"]=="POST"){
        $gender = $_POST['gender'];
        $Jeans_color = $_POST['Jeans_color'];
        $Jeans_brand = $_POST['Jeans_brand'];
        $sql = "Select * from `Jeans` where `gender` = '$gender' and `Jeans_brand`='$Jeans_brand' and `Jeans_color`='$Jeans_color'";
        $result = mysqli_query($conn,$sql);
    }
    ?>
        ...
        <?php
              ...
              function display_items($gender,$result){
                  while($row = mysqli_fetch_assoc($result)){
                      ...
                      $Jeans_color = $row['Jeans_color'];
                      $Jeans_brand = $row['Jeans_brand'];
                      $Jeans_price = $row['Jeans_price'];
                      $item_id =  $gender.'-jeans-' .$Jeans_id;
                      $cat = "Jeans";
                      echo '<div class="view_items"
                      style="border:2px solid white;margin:40px; width:250px;height:400px;position:relative;">
                      <div class="image">
                        <a href="items.php?item_id='.$item_id.'&gender='.$gender.'&brand='.$Jeans_brand.'&price='.$Jeans_price.'&cat='.$cat.'&color='.$Jeans_color.'">
                            <img src="img/Jeans/'. $gender.'-Jeans-'.$Jeans_id.'.png" alt="no image"
                            style="width:250px;height:300px;cursor:pointer;">
                        </a>
                        <p style="font-size:20px"><b> '.$Jeans_brand .'</b><i class="fa fa-regular fa fa-heart"
                        style="float:right;width:25px;height:25px"></i></p>
                        <p>'.$gender.' slim fit Jeans <br>
                        <b>Rs '.$Jeans_price .'</b>
                        </p>
                        </div>
                        <br>
